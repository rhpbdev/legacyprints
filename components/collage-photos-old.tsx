...
export default function CollagePhotoUploads({ userId, memorialId }: Props) {
	...
	const fetchPhotos = useCallback(async (): Promise<CollagePhoto[]> => {
		const folder = encodeURIComponent(`${userId}/${memorialId}/collage-photos`);
	...
	// Load photos on component mount and when dependencies change
		...
		try {
			for (let i = 0; i < selectedFiles.length; i++) {
				...
				try {
					...
					uploadedResults.push(uploadResponse);
					successCount++;
				} catch (error) {
					failedCount++;
			...
			if (successCount > 0) {
				...
				// give ImageKit ~500ms to catch up
			...
			<div className='mt-8'>
				<div className='flex justify-between items-center mb-4'>
					...
					<button
						...
						{isLoadingPhotos ? 'Loading...' : 'Refresh'}
					</button>
				</div>
